    ...
    static Type Check(Unary u, TypeEnv te) {
    	...
    	case "-":
    		if(t1 == Type.INT)
    			...
    			error(u, "Unary - has non-int operand");
    		break;
    	}
    	return u.type;
    }
...
//        ------------------------------
    ...
    // (7) For Type Check Implementation
    static Type Check(For f, TypeEnv te) {
    	// 선언한 decls의 타입을 타입환경에 추가해준다.
    	addType(f.decls, te);
    	// expr, assignment, stmt의 타입을 체크한다.
    	Type t1 = Check(f.expr, te);
    	Type assignment = Check(f.assignment, te);
    	Type t2 = Check(f.stmt, te);
    	
    	// for문에서 expr의 타입은 boolean이어야 하므로
    	// boolean타입이 아닐 경우 error를 실행한다.
    	if(t1 == Type.BOOL) {
    		
    		// for문 실행중 stmt에서 리턴값이 있다면
    		// loop를 수행하지 못하고 종료되기 때문에 error를 실행한다.
    		if(t2 == Type.VOID) {
    			
    			// stmt의 타입이 void를 만족한다면
    			// for문의 타입을 void로 초기화하고 반환한다.
    			f.type = t2;
    		}
    		else
